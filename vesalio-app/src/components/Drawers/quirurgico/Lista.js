...
export default function ListaCitasTabla({ lista: datos }) {
    ...
    const handleRemove = async (id) => {
        let confirm = await confirmSweetAlert('Eliminar antecedente patologico', '¿Esta seguro que desea remover el antecedente patologico?', 'warning', true)
        if (confirm) {
            let obj = {
                documento: user.datos.numero_documento,
                id
            }
            const response = await axios.post(`http://200.121.91.211:4001/removeAntecedenteQuirurgico`, { ...obj })
            if (typeof response.data === 'object') {
                ...
                drawerOpen.item.setDatosAntecedentes('quirurgico')
            }
                ...
                <Table ref={tableRef}>
                    <TableHead>
                        <TableRow>
                            ...
                            <TableCell>{t('Fecha cirugia')}</TableCell>
                    ...
                    <TableBody>
                        {paginatedmedicosLista.map((list) => {
                            return (
                                <TableRow hover key={list.idPatologico} >
                                    <TableCell>
                                        <Typography
                                            noWrap
                                            variant="subtitle1"
                                    ...
                                    <TableCell>
                                        <Typography
                                            ...
                                            variant="subtitle1"
                                            color="text.primary">
                                            <b>{list.fecha_cirugia.split('T')[0].split('-')[2] + '-' + list.fecha_cirugia.split('T')[0].split('-')[1] + '-' + list.fecha_cirugia.split('T')[0].split('-')[0]}</b>
                                        </Typography>
                                    </TableCell>
                                    <TableCell>
                                        <Typography
                                    ...
                                    <TableCell>
                                        <Tooltip title={t('Eliminar')} arrow>
                                            <IconButton
                                                size="small"
                                                color="secondary"
                                                onClick={() => handleRemove(list.idPatologico)}>
