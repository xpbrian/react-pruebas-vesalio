...
export default function Index() {
    ...
    return (
        <>
                ...
                <Grid item lg={12} sx={{ mr: 2 }}>
                    <Button
                        ...
                        onClick={() => mostrarComponent({
                            contenido: 'noPatologicos',
                            estado: true,
                            size: 'sm',
                            item: {
                                datosUsuario: drawerOpen.item.datosUsuario,
                                setDatosAntecedentes: () => drawerOpen.item.setDatosAntecedentes('noPatologicos')
                            }
                        }, 'modalOpen')}
                        startIcon={<AddIcon />}>
                        Agregar
            ...
            <Grid container spacing={1} sx={{ my: 1 }}>
                <Grid item lg={12} sx={{ mr: 2 }}>
                    {lista.length === 0 && <Box sx={{ m: 3 }}><Typography variant='h4'>No registra antecedentes no patológicos</Typography></Box>}
