...
export default function ListaCitasTabla({ lista: datos }) {
    ...
    const handleRemove = async (id) => {
        let confirm = await confirmSweetAlert('Eliminar medicamento habitual', '¿Esta seguro que desea remover el medicamento habitual?', 'warning', true)
        if (confirm) {
            let obj = {
                documento: user.datos.numero_documento,
                id
            }
            const response = await axios.post(`http://200.121.91.211:4001/removeAntecedenteMedicamento`, { ...obj })
            if (typeof response.data === 'object') {
                ...
                drawerOpen.item.setDatosAntecedentes('medicamentos')
            }
