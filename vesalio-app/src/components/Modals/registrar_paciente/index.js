import { Box, Button, Step, StepLabel, Stepper, styled, Zoom, Avatar, Collapse, Alert, IconButton, Typography, Container } from '@mui/material'
...
export default function Index() {
    ...
    const steps = ['Datos RENIEC', 'Datos adicionales', 'Completado']
    ...
    const radios = [
        { value: 'M', label: 'Masculino', step: 0 },
        { value: 'F', label: 'Femenino', step: 0 },
    ...
    const handleChangedText = async (e) => {
        ...
        if (e.target.name === 'numerodocumento') {
            ...
            } else if (e.target.value.length > 8 && textPersonales.find(x => x.id === 'tipodocumento').value === '01') {
    ...
    const handleDate = (e) => {
        setTextPersonales(x => x.reduce((arr, item) => {
            if (item.id === 'fecha') {
                arr.push({ ...item, value: e.target.value })
            } else {
                arr.push(item)
            }
            return arr;
    ...
    const [textPersonales, setTextPersonales] = useState([
        ...
        { id: 'numerodocumento', type: 'number', label: 'Numero de documento', value: '', disabled: false, tipo: 'text', xs: 12, lg: 6, md: 6, onchanged: (e) => handleChangedText(e), step: 0 },
        { id: 'paternos', label: 'Apellido Paternos', type: 'text', value: '', disabled: true, tipo: 'text', xs: 12, lg: 6, md: 6, onchanged: (e) => handleChangedText(e), step: 0 },
        { id: 'maternos', label: 'Apellido Maternos', type: 'text', value: '', disabled: true, tipo: 'text', xs: 12, lg: 6, md: 6, onchanged: (e) => handleChangedText(e), step: 0 },
        ...
        { id: 'fecha', label: '', value: '', type: 'date', disabled: false, tipo: 'date', xs: 12, lg: 6, md: 6, onchanged: (e) => handleDate(e), step: 0 },
        { id: 'genero', label: 'Genero', value: '', type: '', disabled: false, tipo: 'radio', xs: 12, lg: 6, md: 6, onchanged: (e) => handleChangedText(e), step: 0, array: radios.filter(x => x.step === 0) },
    ...
    const datosPersonalesSiguiente = async () => {
        ...
        if (textPersonales.filter(x => x.value.length === 0 && x.step === step).length > 0) {
            ...
            if (textPersonales.find(x => x.id === 'tipodocumento').value === '02' && textPersonales.find(x => x.id === 'numerodocumento').value.length >= 15) {
                errors.push('El Carné de extranjeria solo debe contener como maximo 15 digitos')
            }
    ...
    const datosCuentaSiguiente = async () => {
        ...
        if (errors.length > 0) {
            ...
            if (rpta) {
                const enviar = await axios.post('http://apis-vesalio.com.pe/registroPaciente', {
                    datos: [...envio,
    ...
    return (
        <Box sx={{ p: 2 }}>
            ...
            {
                step === 0 && <DatosPersonales textPersonales={textPersonales.filter(x => x.step === step)}  />
            }
            {
