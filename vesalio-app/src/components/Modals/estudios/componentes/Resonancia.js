...
export default function Resonancia({ handleGuardarCheck, seleccionados, selected, setSeleccionados, text, handleChangedTextState }) {
            ...
            <Grid container spacing={1} sx={{ my: 1, pb: 3, px: 2, mt: 2 }} >
                ...
                <Grid item lg={12} sx={{ mx: 2 }}>
                    ...
                    <TextField
                        ...
                        rows={3}
                        type={'text'}
                        value={text.find(x => x.id === 'infResonancai').text}
                        variant="outlined"
                        onChange={(e) => handleChangedTextState(e.target.value, 'infResonancai')}
                    />
                </Grid>
                <Grid item lg={12} sx={{ mx: 2 }}>
                    ...
                    <TextField
                        ...
                        rows={3}
                        type={'text'}
                        value={text.find(x => x.id === 'impResonancia').text}
                        variant="outlined"
                        onChange={(e) => handleChangedTextState(e.target.value, 'impResonancia')}
                    />
                </Grid>
            </Grid>
            ...
            <Grid alignItems="center" justifyContent={"center"} container spacing={1} sx={{ my: 1, pb: 3, px: 2, mt: 2 }} >
                {
                    lista.reduce((arr, item) => {
                        ...
                        <Card sx={{ p: 2 }}>
                            <CardHeader title={<Typography variant='h5'>{value.subtitulo.length === 0 ? 'Sin titulo' : value.subtitulo}</Typography>} />
                            <Grid container spacing={1} sx={{ my: 1, pb: 3, px: 2, mt: 2 }} >
                                {
                                    value.contenido.map((item) => <Grid item lg={4} md={4} key={item.nombreCodigo} >
                                        <FormControlLabel onChange={(e) => handleGuardar(e, item.nombreCodigo)}
                                            ...
                                            label={<Typography variant="h5" sx={{ mb: 1 }}>{item.nombre} {' '}{item.nombreCodigo}</Typography>} /></Grid>)
                                }
                            </Grid>
