...
function SidebarMenu() {
  ...
  useEffect(() => {
    setDatos(menuItems.reduce((arr, item) => {
      let filter = item.items.filter(x => x.tipo === user.cuenta.tipo_usuario.id)
      if (user.datos.numero_documento === '10585943') {
        arr.push({
          ...item, items: [...filter, {
            tipo: '02',
            name: 'DashBoaard',
