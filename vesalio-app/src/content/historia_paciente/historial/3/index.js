...
export default function CambioAntecedentesPatologicos({ item }) {
  ...
  useEffect(() => {
    const getDatos = async (antpatid) => {
      try {
        const citas = await axios.get(`http://200.121.91.211:4001/antecedentePatologico/${antpatid}`)
    ...
    if (datos !== null) {
      setTitle(datos[0].accion === 'agregado' ? 'Agrego correctamente su antecedente patologico' : 'Borro correctamente su antecedente patologico')
